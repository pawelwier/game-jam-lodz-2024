...
use crate::character::{resources::CharMovement, systems::get_char_animation};

use super::components::{AnimatedEntity, AnimationIndices, AnimationTimer, SpriteLayout};
...
pub fn set_atlas_layout(
    char_movement: ResMut<CharMovement>,
    asset_server: Res<AssetServer>,
    image: &mut Handle<Image>, 
    atlas: &mut TextureAtlas,
    mut texture_atlas_layouts: ResMut<Assets<TextureAtlasLayout>>
) -> () {
    let animated_entity: AnimatedEntity = get_char_animation(
        &char_movement.state,
        asset_server
    );
